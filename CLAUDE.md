...
## Status
Erste 1:1-Übertragung von `index.html`/`submit.php` nach `frontend/` + `backend/` ist geschrieben (alle Felder, Kamera, Unterschrift, PDF, Mailversand, DE/EN, Inaktivitäts-Reset). **Noch nicht verifiziert** — `npm install` konnte auf diesem Mac nicht laufen, weil das globale `~/.npmrc` auf den privaten Azure-Artifacts-Feed der Firma zeigt und dieser gerade ein Kontingent-/Zahlungsproblem hat. Luca installiert/testet auf dem Windows-Rechner. **Erster Schritt dort: `npm install` in `frontend/` und `backend/`, dann `npm run build` bzw. `tsc -b --noEmit`, um Typfehler zu finden, die hier nicht geprüft werden konnten.**
...
- **`redux/`-Ordner bleibt vorerst leer/ungenutzt.** Für eine einzelne Formularseite reicht React-State (`useState` in `App.tsx`), das entspricht dem Original (dort war auch alles in einem Scope). Die Guideline verlangt Redux nur für globalen/geteilten State über mehrere Komponenten hinweg — falls das Team das trotzdem von Anfang an will, sagen und wir bauen es um.
